...
- [x] 1.1 Create `pyproject.toml` with package metadata, Python ≥3.10 requirement, setuptools backend, runtime dependencies (piecash ≥1.2.0, weasyprint ≥60.0, Jinja2 ≥3.1), test dependencies (pytest ≥7.0), and `bokforing` console script entry point
- [x] 1.2 Create `bokforing/__init__.py` with package version string
- [x] 1.3 Create `bokforing/__main__.py` that delegates to `cli.main()` for `python -m bokforing` support

## 2. Data Models

- [x] 2.1 Create `bokforing/models.py` with custom exception hierarchy: `BokforingError`, `CSVParseError`, `GnuCashError`, `RulesDBError`
...
- [x] 3.1 Create `bokforing/cli.py` with a `main()` function stub that prints a usage message (full argparse logic deferred to Phase 4)
