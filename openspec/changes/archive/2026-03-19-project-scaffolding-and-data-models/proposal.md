...
- Create `pyproject.toml` with package metadata, dependencies (piecash, weasyprint, Jinja2, pytest), and entry point (`bokforing` CLI command)
- Create the `bokforing/` package directory with `__init__.py` and `__main__.py`
- Implement all shared data models in `bokforing/models.py`: `BankTransaction`, `CategorizationSuggestion`, `JournalEntrySplit`, `JournalEntry`, `Rule`, `CompanyInfo`, `ImportResult`, and the `CSVParseError` exception
...
- **Code**: Creates the entire `bokforing/` package and `tests/` directory from scratch
